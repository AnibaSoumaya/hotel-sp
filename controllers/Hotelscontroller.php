...
class Hotelscontroller extends Connexion{
    ...
    function __construct() {
        ...
        public function getCountHotels() {
            ...
            public function searchHotelsByLocation($location) {
                // Préparez la requête SQL pour rechercher les hôtels par emplacement
                $query = "SELECT * FROM hotels WHERE location LIKE ?";
                $stmt = $this->conn->prepare($query);
                // Ajoutez des wildcards (%) autour de l'emplacement pour une recherche partielle
